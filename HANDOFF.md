...
**キャラクター実装の進捗**:
- **画像化済み4体（ひのこ・しずく・はっぱ・ぴかり）**: 画像化完了・12枚が配信対象
- **その他5体（もくも・きらら・がんろ・こおる・かげろ）**: SVG 生成のまま
...
### C2. レベル曲線（2026-07-31変更）

全キャラクターを育てやすくするため、必要EXPを旧曲線のおよそ10%へ短縮しました。

```js
expToNext(level) = Math.max(1, Math.round(0.3 * level ** 1.8))
```

- Lv100到達は **42,061 EXP**（旧曲線は420,590 EXP）
- Lv100未満では、丸め後も最低1 EXP必要
...
**症状**: 記録を削除したのに EXP が増える（旧曲線当時の正例: はっぱ exp 4384・ワン64/ノー166/ノー192/ノー42 で、ノー166 を削除すると +78）。
...
### リポジトリの公開方針（2026-07-31 決定）

**当面は public を維持する。** `liftingmaster` Organization は GitHub Free であり、現在の
GitHub Pages 本番を無料かつ単純な構成で継続するため。feature ブランチと Pull Request も
public になるので、`main` へ入れない途中の変更も外部から閲覧・複製できる前提で扱う。

この判断は「コードを秘密情報として扱わない」という意味であり、利用者データの公開を許す
ものではない。**次のものはブランチを問わず commit / push しない。**

- お子さんの記録を書き出した JSON、画面写真などの利用者データ
- 生の親パスワード、認証トークン、API キー、秘密鍵
- 外部公開を想定していない氏名、メールアドレスなどの個人情報

お子さんの記録と親パスワードのハッシュは端末の `localStorage` にだけ保存する現在の設計を
維持する。PWA の HTML / CSS / JavaScript は利用者のブラウザへ配信する必要があるため、将来
リポジトリを private にしても、配信中のフロントエンドコード自体を完全に隠すことはできない。

**再検討する条件:** 商品化、バックエンドや外部 API の導入、公開したくない設計資料の追加、
複数人での開発開始。このいずれかが起きたら、GitHub Team で現 repo を private にするか、
private な開発 repo と public な配布先を分離する。GitHub Free のまま現 repo を private にすると
本番 Pages が停止するため、移行計画なしに公開範囲を変更してはいけない。過去に public だった
履歴は、private 化しても既に取得されたコピーまで回収できない。

**ビジョン照合:** ビジョン外（運用方針）。本番継続と引き継ぎ時の情報漏えい防止のための判断。
...
### public だが秘密情報・利用者データは置かない

コードとキャラ画像だけで、**お子さんの記録がここに入ることはありません**（記録は端末の
localStorage のみ）。親のパスワードもハッシュで端末内にしかありません。公開方針と
commit 禁止事項は上記「リポジトリの公開方針」を正本とします。
