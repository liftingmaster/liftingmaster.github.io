...
現在 **437テスト全通過** / `CACHE_NAME = liftingmaster-v19` / `SCHEMA_VERSION = 2`
...
- **デプロイ版**: `liftingmaster-v19`
- **テスト**: 437件全通過
- **本番稼働**: 安部さんのお子さんが毎日使用中

**キャラクター実装の進捗**:
- **画像化済み5体（ひのこ・しずく・はっぱ・ぴかり・もくも）**: 画像化完了・15枚が配信対象
- **その他4体（きらら・がんろ・こおる・かげろ）**: SVG 生成のまま
...
1. **キャラ画像の差し替え（4体残）** — 画像ができているのは**5体・15枚**
   （`js/img/` の中身と `sw.js` の `ASSETS` で確認できる）。
   **ぴかり・もくもは画像化完了**（ぴかりのプロンプトは `docs/art-prompts/pikari.md`）。
   きらら・がんろ・こおる・かげろ の4体はプロンプトも未作成。
   - プロンプト作成 → `node tools/prepare-art.js` → 目視確認 → `js/svg/artManifest.js` 登録 → `sw.js` 追加 → `CACHE_NAME` 上げる
   - 残り4体を追加すると合計約5MBになる見込み
...
CI / Actions    test workflow（必須check名: node-test）
                → Pull Request と main で全437テストを実行。PRでは CACHE_NAME の版上げも検査
