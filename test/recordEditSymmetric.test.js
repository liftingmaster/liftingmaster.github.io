...
// 2026-07-29（欠陥Aの是正・当初方針への差し戻し）: before は常に保存値
// （isLegacyDay による日ごとの帳簿切り替えは廃止）。実装は未変更のため、
// このテストは新しい期待値に対して現状の実装では FAIL する想定（それが目的）。
//
// 最終手（n2を55→56）の新しい導出:
//   その時点で exp=0（o1,o2削除でクランプの底）。n1.grantedExp=150,
//   n2.grantedExp=15 は「保存値」としてはそのまま残っている（別グループの
//   削除ではn1・n2自身の保存値は書き換わらない）。
//   before = n1(保存値150) + n2(保存値15) = 165（常に保存値、リプレイし直さない）
//   after  = baseExp(=max(0,0-165)=0)からグループ[n1,n2]を引き直す:
//     n1(count=50,charExp=0,acc=[])=225 / n2(count=56,charExp=225,acc=[n1])=27
//     → after合計=252
//   diff = 252 − 165 = +87 → exp = max(0, 0+87) = 87
// クランプで失われた負債(-105,-315)が「保存値どおりに取り消す」設計のもとでは
// 別グループのクランプ由来の水増しとして残ってしまう（このテスト名が指す
// 「無からEXPが生まれない」を完全には満たせない残存事象）。これは
// baseExpOf 自身が Math.max(0, …) で内部クランプを持つこと（settleDayの外側で
// 別途クランプしている）に起因し、before方針の選択（保存値かリプレイ値か）
// では解消できない、別種の穴として報告する（後述コメント参照）。
test('I5 ネットクランプ: 旧データを2件消してクランプに当たった後、無関係な兄弟の微調整で無からEXPが生まれない（2026-07-29差し戻し後の実測値。完全解消はしていない旨は報告に記載）', () => {
  const p = base('hinoko');
  ...
  // o1は保存値を持たない旧データなので、beforeはリプレイ推定にフォールバックする
  // （常に保存値、というルールの唯一の例外＝額が保存されていないデータ）。
  ...
  assert.equal(charExp(player, 'hinoko'), 87, '常に保存値方針での実測値（現状の実装は4を返す想定。実装がisLegacyDay分岐を残したままだとFAILする）');
  assert.equal(result.expDelta, 87);
});
...
// =============================================================================
// S1〜S4: 連続編集。2026-07-29（欠陥Aの是正・当初方針への差し戻し）で
// 前提が変わったセクション。
//
// 旧コメント（2026-07-28時点）はここを「別グループの残留という既知の割り切り」
// として固定し、「あるべき値」との差分をコメントに書いたうえで棚上げしていた。
// これは before を「編集対象グループのリプレイ値」で作っていたため。
//
// 2026-07-29、before を「常に、そのキャラに紐づく grantedExp の保存値の合計」に
// 統一したことで、この割り切りは**解消される**。deleteRecord は「そのレコードに
// 実際に付与されていた額（保存値）をそのまま取り消す」ので、削除対象のレコードが
// 別グループ（別の日）でどんな計算をされていたかに関わらず、常に「あるべき値」
// （このグループが最初から無かったのと同じ状態）に一致する。
// 実際、下のS1〜S4の期待値はすべて、削除前のコメントが「あるべき」と書いていた
// 値そのものになった（S1: 90、S2: 2250・Lv15、S3: 45）。実装(js/core/player.js)は
// まだ変更していないため、これらのテストは現状の実装に対して FAIL する想定
// （それが目的。手計算はscratchpad上の別実装の電卓で検証済み）。
// =============================================================================

test('S1 連続編集（2026-07-29差し戻し後は割り切りが解消。あるべき値=90になる）: ひのこ 7/1を5→20に直してから7/5を消すと exp=90 になる', () => {
  const p = base('hinoko');
  ...
  // before は常に保存値(36)なので、削除は「r2に実際に付与されていた36をそのまま
  // 取り消す」だけになる。90(7/1単独) − 36(取り消し) + 36(before) は自明に90に戻る
  // （before=after=保存値の場合の削除は「保存値ぶんだけ引く」）。
  // 2026-07-29差し戻し前は「あるべきは90」とコメントされていたが、実測は102だった
  // （+12の残留）。差し戻し後はその残留が解消され、あるべき値どおり90になる
  assert.equal(charExp(step2.player, 'hinoko'), 90, '2026-07-29差し戻し後: 割り切りが解消され、あるべき値どおり90になる（現状の実装は102を返す想定）');
...
test('S3 連続編集（2026-07-29差し戻し後は割り切りが解消。あるべき値=45になる）: ひのこ 400→10 のあと 300 を削除で exp=45 になる', () => {
  ...
  const step2 = deleteRecord(step1.player, { recordId: 'r2', now: NOW });
  // before は常に r2 の保存値(900)。945 − 900 = 45（7/1単独分）にちょうど一致し、
  // クランプには当たらない。2026-07-29差し戻し前は「あるべきは45」とコメントされて
  // いたが、実測は0（負債が持ち越されてクランプ）だった。差し戻し後は解消される
  assert.equal(charExp(step2.player, 'hinoko'), 45, '2026-07-29差し戻し後: 割り切りが解消され、あるべき値どおり45になる（現状の実装は0を返す想定）');
});

// 2026-07-29（欠陥Aの是正）: S4はS3と同じ2手を土台にしていたため、S3の変更
// （0にクランプ→45）に連動して土台の値が変わる。3手目・4手目の期待値も
// 新方針（beforeは常に保存値）でのリプレイに合わせて導出し直した
test('S4 連続編集（2026-07-29差し戻し後の期待値）: 何度編集を重ねても exp と grantedExp は負にならない', () => {
  const p = base('hinoko');
  p.chars[0].exp = 1800 + 900;
  p.records = [
    {
  ...
  // S3と同じ2手（945 → 45。S3参照）のあと、さらに減らす・増やすを繰り返す
  const step1 = editRecord(p, { recordId: 'r1', count: 10, now: NOW });
  const step2 = deleteRecord(step1.player, { recordId: 'r2', now: NOW });
  assert.equal(charExp(step2.player, 'hinoko'), 45, 'S3と同じ土台（あるべき値・現状の実装は0を返す想定）');

  // 3手目: exp=45の状態でさらに 10→1 に減らす。r1(唯一のグループ)を空の状態から
  // 引き直す: before=45(r1単独の保存値と一致) / after=round(1×3×1.5)=5 → diff=-40
  const step3 = editRecord(step2.player, { recordId: 'r1', count: 1, now: NOW });
  assert.equal(charExp(step3.player, 'hinoko'), 5, '45-40=5。クランプに当たらない（現状の実装は0を返す想定）');
  assert.equal(step3.result.expDelta, -40);
  ...
  // before=保存値5 / after=100×3×1.5=450 → diff=+445 → exp=5+445=450
  const step4 = editRecord(step3.player, { recordId: 'r1', count: 100, now: NOW });
  assert.ok(charExp(step4.player, 'hinoko') >= 0);
  assert.equal(charExp(step4.player, 'hinoko'), 450, '5+445=450（現状の実装は445を返す想定）');
